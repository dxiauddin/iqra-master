...
export default function ModuleTestPage({ params }: PageProps) {
  ...
  const [isSharing, setIsSharing] = useState(false); // Controls hiding the close button during capture
  ...
  // Theme configuration perfectly matching exact Skala gradients and accents
  ...
  const handleAnswerSelect = (optionName: string) => {
    ...
    }, 500); // 500ms for fast mobile audio response
  };
  ...
  const handleShareWhatsApp = async () => {
    ...
    try {
      setIsSharing(true); // Hide close button before snapshot capture
      await new Promise((resolve) => setTimeout(resolve, 100)); // Let UI settle

      // Sharp HD rendering with pixelRatio: 3
      const dataUrl = await htmlToImage.toJpeg(resultCardRef.current, { 
        quality: 0.98,
        pixelRatio: 3 
      });
      
      setIsSharing(false); // Restore close button
      ...
      {isCompleted && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-start justify-center pt-6 px-4 pb-20 overflow-y-auto font-sans">
          <div className="w-full max-w-sm flex flex-col items-center space-y-3 animate-in fade-in zoom-in duration-300 relative my-auto">
            ...
            {/* Printable Certificate Card Wrapper with Multi-Color Spinning Border Beam (Attached to resultCardRef) */}
            <div ref={resultCardRef} className="relative p-[1.5px] rounded-[26px] overflow-hidden w-full shadow-2xl bg-zinc-950">
              <div className="absolute inset-[-150%] bg-[conic-gradient(from_0deg,#3B82F6,#FACC15,#EF4444,#3B82F6)] animate-spin-slow pointer-events-none" />

              <div className={`relative w-full bg-gradient-to-br ${theme.cardBg} p-5 rounded-[24px] text-white text-center border border-white/15 space-y-3`}>
                
                {/* Close (✕) Button - Hidden dynamically during image export capture */}
                {!isSharing && (
                  <Link
            ...
            <div className="flex flex-col w-full space-y-2">
              <button
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-medium py-2.5 px-6 rounded-full text-[15px] transition-transform hover:scale-105 shadow-lg flex items-center justify-center gap-2.5 cursor-pointer"
              >
